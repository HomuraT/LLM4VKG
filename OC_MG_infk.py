"""
OC_MG_infk.py - 本体补全和映射生成主程序（使用 infk 数据版本）
...
- 使用基于 infk 的映射模式（从 outputs-1103/mapping_patterns_infk/ 读取）
- 输出到不同的目录（避免覆盖原始结果）
...
if __name__ == "__main__":
    ...
    mapping_pattern_dir = 'outputs/mapping_patterns_infk/'
    
    # infk table_info 的输入目录（包含完整外键信息）
    table_info_input_dir = 'infk/'
        ...
        for subset_name in subset_names:  
            ...
            for dbname in tqdm(dbnames, desc=f"{subset_name} dbname (infk)"):
                try:
                    ...
                    base_path = f'./outputs/{subset_name}/LLM4VKG_{api_name}_infk/' + dbname
                    # 定义生成的本体文件名
                    ...
                    if not os.path.exists(mapping_pattern_path):
                        ...
                        print(f"请先运行 MPR_infk.py 生成基于 infk 的映射模式")
                        continue
                    ...
                    print(f"跳过数据库 {dbname} 的处理")
                    # 可选：记录详细的错误信息到日志文件
                    error_log_path = os.path.join('outputs', 'error_log_infk.txt')
